...
const key = "qEEiL2tRbwN5WP@%Ku3jv3Bjf&u*B!fw"
...
function decrypt(text) {
  ...
  let encryptedText = Buffer.from(text.ciphertext, 'hex');
    ...
    onHeaders(res, function setHeaders() {
      ...
      try {
        if (sess === false) {
          ...
          debug('save %s', name)
          cookies.set(name, encode(encrypt(Session.serialize(sess))), req.sessionOptions)
        }
      } catch (e) {
        console.log('error saving session %s', e.message)
      }
    })
